...
class es_imagecls
{
	 ...
	 * @return bool
	 */
	function init($file, $dir = 'attachment')
	{
		...
		{	
			...
			$tmpExt = substr($file['name'], strlen($file['name'])-3);
			$isfileExt = explode(",", app_conf('ALLOW_IMAGE_EXT'));
			
			if(!in_array(strtolower($tmpExt), $isfileExt)){
				$this->error_code = -200;
				return false;
			}
			$ext = ".".$tmpExt;
			...
			$file['target'] = "/public/".$file['file_dir'].'/'.$file['prefix'].$ext;  //相对
			$file['local_target'] = ROOT_PATH."public/".$file['file_dir'].'/'.$file['prefix'].$ext;  //物理
			$this->file = &$file;
	...
	{
        if (!is_dir(ROOT_PATH."public/".$dir)) {
             @mkdir(ROOT_PATH."public/".$dir);
             @chmod(ROOT_PATH."public/".$dir, 0777);
        }
        $dir .= "/".to_date(TIME_UTC,"Y");
        if (!is_dir(ROOT_PATH."public/".$dir)) {
        	@mkdir(ROOT_PATH."public/".$dir);
        	@chmod(ROOT_PATH."public/".$dir, 0777);
        }
        $dir .= "/".to_date(TIME_UTC,"m");
        if (!is_dir(ROOT_PATH."public/".$dir)) {
        	@mkdir(ROOT_PATH."public/".$dir);
        	@chmod(ROOT_PATH."public/".$dir, 0777);
        }
        $dir .= "/".to_date(TIME_UTC,"d");
        if (!is_dir(ROOT_PATH."public/".$dir)) {
        	@mkdir(ROOT_PATH."public/".$dir);
        	@chmod(ROOT_PATH."public/".$dir, 0777);
        }
	...
	public function thumb($image,$maxWidth=200,$maxHeight=50,$gen = 0,$interlace=true,$filepath = '',$is_preview = true)
    {
        ...
        if($info !== false)
		{
			...
			$thumburl = str_replace(ROOT_PATH,'./',$thumbname);
	...
	public function water($source,$water,$alpha=80,$position="0")
    {
        ...
        {
        	require_once "./gif_encoder.php";
			require_once "./gif_reader.php";
