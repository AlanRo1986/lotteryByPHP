...
{//类定义开始
     ...
     * @return string
     +----------------------------------------------------------
     */
    static function buildImageVerify($length=6,$mode=1,$type='gif',$width=150,$height=39,$verifyName='verify')
    {
        ...
        $fontfile = PUBILC_PATH. 'Verify/ttfs/'.mt_rand(1,6).'.ttf';
		...
		for($i=0;$i<$length;$i++) {
			if($width >= 89){
				//imagestring($im,15,$i*10+25,mt_rand(15,8),$randval{$i}, $stringColor);
			    imagettftext($im, mt_rand(18, 28), 0, $i*20+20, mt_rand(20,40), $borderColor, $fontfile, $randval{$i});
